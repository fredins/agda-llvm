...
% TODO fix ugly
% TODO fix subjective statements 'excessive'
...
% TODO 
% • Correctness. The orignal Perceus algorithm by Reinking et al. is formalized and proven \emph{garbage free}. We have not formalized our variant of the algorithm. Still, the algorithm have been remarkably robust and predictable.
...
We can prevent the stack overflow by making \AgdaFunction{sum} tail recursive and eager in the first argument, by using the primitive \mbox{\AgdaFunction{primForce}}.
Evaluating (\AgdaBound{n} \AgdaFunction{+} \AgdaBound{m}) prior to the recursive call, achieves several favorable properties.
Firstly, we prevent the stack overflow by not building and then evaluating a large suspended computation.
Secondly, forcing \AgdaFunction{\AgdaUnderscore+\AgdaUnderscore} drops the last reference count to \AgdaBound{n}, which means it can be freed directly.
...
% TODO maybe write that we need as strictnes analysis and the program should not be required to know the intricacies of  
%      the reference counting algorithm.

% TODO maybe write something about other metrics such as compile times and code size.

% TODO discuss extending the heap points-to analysis to also handle parametric polymorphishm and adhoc polymorphishm. Then we could 
%      combine this with specialization to achive a good balance of branching and specialization.
